...
// Build the Hono app with all API routes. Shared by the node-server entry
// point (main.ts) and serverless adapters (e.g. frontend/pages/api/*.ts).
export const createApp = (repoStore: RepoStore) => {
  const app = new Hono();
  app.use(cors());
  app.use(compress());
  ...
  // Redirects to the canonical URL so the CDN caches one entry per unique chart.
